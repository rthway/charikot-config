...
INNER JOIN coded_obs_view AS malaria_type ON encounter.encounter_id = malaria_type.encounter_id
	AND malaria_type.concept_full_name = 'Malaria note-Malaria note type'
LEFT OUTER JOIN coded_obs_view AS pregnancy_status ON pregnancy_status.person_id = person.person_id
	AND pregnancy_status.concept_full_name = 'Malaria note-Pregnant'
...
(SELECT
	malaria_types_list.name_value AS 'Type of Malaria note',
...
LEFT OUTER JOIN coded_obs_view AS malaria_classification ON malaria_classification.person_id = person.person_id
	AND malaria_classification.concept_full_name = 'Malaria note-Classification'
    AND malaria_classification.obs_datetime BETWEEN @start_date AND @end_date
) AS entries
RIGHT OUTER JOIN (SELECT name_key, name_value, sort_order FROM row_header_name_map WHERE report_group_name = 'Types of Malaria note' ) AS malaria_types_list ON entries.malaria = malaria_types_list.name_key
...
RIGHT OUTER JOIN (SELECT name_key, name_value, sort_order FROM row_header_name_map WHERE report_group_name = 'Malaria note-No of malaria cases' ) AS malaria_types_list ON malaria_type.value_concept_full_name = malaria_types_list.name_key
GROUP BY malaria_types_list.name_value
ORDER BY malaria_types_list.sort_order)
...
RIGHT OUTER JOIN (select answer_concept_name from concept_answer_view where question_concept_name = 'Malaria note-Finding' ) AS malaria_types_list ON malaria_type.value_concept_full_name = malaria_types_list.answer_concept_name
...
INNER JOIN obs_view AS malaria_death ON malaria_death.person_id = person.person_id
	AND malaria_death.concept_full_name = 'Malaria note-Death date'
 ...
 LEFT OUTER JOIN coded_obs_view ON coded_obs_view.person_id = person.person_id
	AND coded_obs_view.concept_full_name = 'Malaria note-Finding'
...
LEFT OUTER JOIN coded_obs_view AS malaria_finding
    ...
    AND malaria_finding.concept_full_name = 'Malaria note-Finding'
