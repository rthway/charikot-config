    ...
    FROM
        ...
        FROM
            ...
            TIMESTAMPDIFF(MONTH, p.birthdate, v.date_started) AS age,
                ...
                IF(oAdtType.obs_datetime >= DATE('2017-04-14'), p.person_id, NULL) thisMonthPatient,
                IF(oAdtType.obs_datetime < DATE('2017-04-14'), p.person_id, NULL) lastMonthPatient
        FROM
            person p
        ...
        LEFT JOIN imam im ON 1 = 1
        WHERE
            ! p.voided AND ! v.voided AND ! e.voided
                AND DATE(oAdtType.obs_datetime) BETWEEN DATE_SUB(DATE('2017-04-14'), INTERVAL 1 MONTH) AND DATE('2018-05-14')
        ...
        -- ORDER BY  `Children at End of Last Month` DESC
        ...
        LEFT JOIN nonVoidedQuestionAnswerObs currentObs ON currentObs.person_id = prevObs.person_id
            ...
            AND DATE(currentObs.obs_datetime) <= '2018-05-14'
        ...
        WHERE
            currentObs.obs_id IS NULL
                AND prevObs.obs_datetime >= DATE_SUB('2017-04-14', INTERVAL 28 DAY)
                AND DATE(prevObs.obs_datetime) <= DATE_SUB('2018-05-14', INTERVAL 28 DAY)
