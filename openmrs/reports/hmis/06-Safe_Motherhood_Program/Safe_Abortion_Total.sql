...
FROM
    ...
    INNER JOIN concept_name answer_concept_fully_specified_name ON ca.answer_concept = answer_concept_fully_specified_name.concept_id
        ...
        AND answer_concept_fully_specified_name.name NOT IN ('NA')
    ...
    ORDER BY answer_name DESC) first_answers
        INNER JOIN
    reporting_age_group rag ON rag.report_group_name = 'Safe Abortion'
        LEFT OUTER JOIN
    (SELECT 
    ...
    person p ON first_concept.person_id = p.person_id
        ...
        AND rag.report_group_name = 'Safe Abortion'
GROUP BY  rag.name ,first_answers.category
